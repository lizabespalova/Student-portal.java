...
public class Helpbot extends TelegramLongPollingBot {
    ...
    @Autowired
    private BotHasQueryCommand.CommandCallbackQueryFactory commandCallbackQueryFactory;
    @Autowired
    private BotHasMessageCommand.CommandHasMessageFactory commandHasMessageFactory;
    @Autowired
    private BotHasNotNullMessageCommand.CommandHasNotNullMessageFactory commandHasNotNullMessageFactory;
...
//        const_text= new Text();
        if (update.hasCallbackQuery()) {
            commandCallbackQueryFactory.getCommand(update).resolve(update);
        }
        ...
        if (update.hasMessage()) {
            commandHasMessageFactory.getCommand(update).resolve(update);
        }
        if (message != null) {
            ...
            if (user_sms != null) {
                ...
                commandHasNotNullMessageFactory.getCommand(update).resolve(update);
            }
        }
    }
